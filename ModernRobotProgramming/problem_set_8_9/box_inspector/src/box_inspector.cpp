...
BoxInspector::BoxInspector(ros::NodeHandle* nodehandle) : nh_(*nodehandle) { //constructor
        ...
        box_camera_subscriber_2_ = nh_.subscribe("/ariac/box_camera_2", 1, &BoxInspector::box_camera_callback_2, this);
...
bool BoxInspector::post_dropoff_check(vector<osrf_gear::Model> desired_models_wrt_world, vector<osrf_gear::Model> &misplaced_models_desired_coords, vector<osrf_gear::Model> &misplaced_models_actual_coords, int cam_num) {
        vector<osrf_gear::Model> satisfied_models_wrt_world, missing_models_wrt_world, orphan_models_wrt_world;
        if (!update_inspection(desired_models_wrt_world, satisfied_models_wrt_world, misplaced_models_actual_coords, misplaced_models_desired_coords, missing_models_wrt_world, orphan_models_wrt_world, cam_num)) {
                return 0;
        }
...
bool BoxInspector::pre_dropoff_check(inventory_msgs::Part part, osrf_gear::Model misplaced_model_actual_coords, osrf_gear::Model misplaced_model_desired_coords, int cam_num) {
        ...
        if (!update_inspection(desired, satisfied_models_wrt_world, misplaced_models_actual_coords, misplaced_models_desired_coords, missing_models_wrt_world, orphan_models_wrt_world,cam_num)) {
                return 0;
        }
...
void BoxInspector::box_camera_callback_2(const osrf_gear::LogicalCameraImage::ConstPtr & image_msg) {
        if (!got_new_snapshot_2_) {
                ...
                //ROS_INFO_STREAM("received box-camera image of: " << box_inspector_image_ << endl);
                int n_models = box_inspector_image_2_.models.size();
                //ROS_INFO("%d models seen ", n_models);
        }
}
...
//method to request a new snapshot from logical camera; blocks until snapshot is ready,
// then result will be in box_inspector_image_2_

bool BoxInspector::get_new_snapshot_from_box_cam_2() {
        got_new_snapshot_2_ = false;
        double dt = 0.05;
        double timer = 0.0;
        while (!got_new_snapshot_2_ && (timer < BOX_INSPECTOR_TIMEOUT)) {
                ros::spinOnce();
                ros::Duration(dt).sleep();
                timer += dt;
        }
...
bool BoxInspector::update_inspection(vector<osrf_gear::Model> desired_models_wrt_world,
                                     ...
                                     vector<osrf_gear::Model> &orphan_models_wrt_world, int cam_num) {
        int ans; // FOR DEBUG
        osrf_gear::LogicalCameraImage filtered_box_camera_image;

        if (cam_num == 1 && !get_filtered_snapshots_from_box_cam(filtered_box_camera_image)) {
                return false;
        } else if(cam_num == 2 && !get_filtered_snapshots_from_box_cam_2(filtered_box_camera_image)) {
                return false;
        }
...
bool BoxInspector::update_inspection(vector<osrf_gear::Model> desired_models_wrt_world,
                                     ...
                                     vector<int> &part_indices_precisely_placed, int cam_num) {
        int ans; // FOR DEBUG
        osrf_gear::LogicalCameraImage filtered_box_camera_image;
        ...
        bool badPartInCam = false;

        if(cam_num == 1) {
                badPartInCam = get_bad_part_Q1(bad_part);
        } else if(cam_num == 2) {
                badPartInCam = get_bad_part_Q2(bad_part);
        }
        ...
        if (badPartInCam) {
                ...
                if(cam_num == 1) {
                        ...
                        ROS_INFO("no bad parts reported by quality sensor 2");
                }
        }
...
bool BoxInspector::get_box_pose_wrt_world(int stationNum, geometry_msgs::PoseStamped &box_pose_wrt_world) {
        if (stationNum == 1) {
                get_box_pose_wrt_world_1(box_pose_wrt_world);
        } else if(stationNum == 2) {
                get_box_pose_wrt_world_2(box_pose_wrt_world);
        }
}

bool BoxInspector::get_box_pose_wrt_world_1(geometry_msgs::PoseStamped &box_pose_wrt_world) {
