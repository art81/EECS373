...
void odomCallback(const nav_msgs::Odometry& odom_msg) {
        ...
        //Makes sure that g_phi is always less than 2pi and always positive
        while (abs(g_phi) >= 2*M_PI || g_phi < 0) {
              if(g_phi > 0) {
                  g_phi -= 2*M_PI;
              } else {
                  g_phi += 2*M_PI;
...
double determineAngleOfWallOnLeft() {
        ...
        AvgLeft   = averagePings(30, pingLeftIdx);
        AvgLeftUp = averagePings(30, pingLeftUpIdx);

        ROS_INFO("AVGLEFT: %f, %f",AvgLeft.xAvg, AvgLeft.yAvg);
        ROS_INFO("AVGLEFTUP: %f, %f",AvgLeftUp.xAvg, AvgLeftUp.yAvg);
        ...
        //If the distLeftUp is off of the wall then dont turn the robot the calculated degrees
        if (distLeftUp - distLeft > WALL_FOLLOW_RADIUS) {
                ros::spinOnce(); //update g_phi
                ROS_INFO("INVALID");
                return 0; //So that the robot doesn't turn at all since we got no real info about the wall
        } else {
                ROS_INFO("VALID");
                double angle = atan((AvgLeftUp.yAvg - AvgLeft.yAvg)/(AvgLeftUp.xAvg - AvgLeft.xAvg)) - (M_PI/2); //added pi/2 so that this angle has 0 being vertical/parallel to the bot.

                //To make sure that the wall angle is always positive.
                if(angle < 0) {
                    angle += 2*M_PI;
                }
...
int main(int argc, char **argv) {
        ...
        while (ros::ok()) {
                ...
                if ((g_radius_min < WALL_FOLLOW_RADIUS) && (g_index_min_dist_ping < g_index_tangent_left)) {
                        ...
                        //This code will first determine at what angle the wall is at in reference to the robot
                        double angleToTurn = determineAngleOfWallOnLeft();
                        ROS_INFO("TURN %f RADIANS TO BE PARALLEL", angleToTurn);

                        //int ans;
                        //cout<<"Press 1 to continue: "; //poor-man's breakpoint
                        //cin>>ans;
                        ...
                        while(abs(abs(g_phi - currPhi) - angleToTurn) >= 0.05) { //while robot angle is further than 0.05 radians away from that of the left wall
